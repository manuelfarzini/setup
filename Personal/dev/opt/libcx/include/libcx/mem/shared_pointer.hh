...
template<typename T> struct control_block_inplace : control_block_base {
  ...
  static cexpr control_block_vtable vtable = {
  ...
  ///
  finline cexpr auto storage_ptr() noexce -> T* {
    return reinterpret_cast<T*>(object_storage);
  }

  ///
  finline cexpr auto object_ptr() noexce -> T* {
...
template<typename T> struct shared_pointer {
  ...
  ///
  static cexpr adopt_tag_t adopt_tag{};
  ...
  ///
  cexpr shared_pointer(shared_pointer&& that) noexce
      : ctrl_ptr(that.ctrl_ptr), obj_ptr(that.obj_ptr) {
  ...
  cexpr shared_pointer(shared_pointer<U> const& that) noexce
  ...
  ///
  template<typename U, typename = std::enable_if_t<std::is_convertible_v<U*, T*>>>
  cexpr shared_pointer(shared_pointer<U>&& that) noexce
      : ctrl_ptr(that.ctrl_ptr), obj_ptr(static_cast<T*>(that.obj_ptr)) {
    that.ctrl_ptr = nullptr;
    that.obj_ptr = nullptr;
  }
  ...
  ///
  finline cexpr auto object_ptr() const noexce -> T* { return obj_ptr; }

  ///
  finline cexpr auto operator*() const noexce -> T& {
  ...
  ///
  finline cexpr explicit operator bool() const noexce {
    return obj_ptr != nullptr;
  }

  ///
  finline cexpr auto use_count() const noexce -> usize {
...
template<typename T> struct weak_pointer {
  ...
  ///
  finline cexpr auto expired() const noexce -> bool {
    return use_count() == 0;
  }
...
template<typename T> finline cexpr T& take(Maybe<T>& m) {
  return m.value();
}
...
template<typename T> finline cexpr cx::mem::shared_pointer<T>&&
