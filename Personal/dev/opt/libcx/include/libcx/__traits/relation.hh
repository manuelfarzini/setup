/// \file libcx/__traits/relation.hh
#ifndef CX___TRAITS_RELATION_HH
#define CX___TRAITS_RELATION_HH
...
template<typename T, typename U> predicate is_convertible = requires { static_cast<U>(cx::declval<T>()); };
// template<typename T, typename U> predicate is_convertible = __is_convertible(T, U);
...
predicate is_implicit_convertible = requires { accept__implicit_convertible<U>(cx::declval<T>()); };

template<typename T, typename U>
using ternary_res = decltype(false ? cx::declval<T>() : cx::declval<U>());

}       // namespace cx
#endif  // CX___TRAITS_RELATION_HH
