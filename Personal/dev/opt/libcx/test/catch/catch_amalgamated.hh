...
#            define CATCH_INTERNAL_IGNORE_BUT_WARN(...)                                            \
                (void)                                                                             \
                    __builtin_constant_p(__VA_ARGS__) /* NOLINT(cppcoreguidelines-pro-type-vararg, \
                                                         hicpp-vararg) */
#        else
#            define CATCH_INTERNAL_IGNORE_BUT_WARN(...)
#        endif
...
enum class ShowDurations : uint8_t
{
    DefaultForReporter,
    Always,
    Never
};
enum class TestRunOrder : uint8_t
{
    Declared,
...
CompleteType_t<FunctionReturnType<Fun, Args...>> complete_invoke(Fun && fun, Args &&... args)
{
    return CompleteInvoker<
        FunctionReturnType<Fun, Args...>>::invoke(CATCH_FORWARD(fun), CATCH_FORWARD(args)...);
}
...
struct ExecutionPlan
{
    ...
    {
        // warmup a bit
        Detail::run_for_at_least<
            Clock>(std::chrono::duration_cast<IDuration>(warmup_time),
                   warmup_iterations,
                   Detail::repeat(
                       []()
                       {
...
struct Benchmark
{
    ...
    ExecutionPlan prepare(const IConfig & cfg, Environment env)
    {
        ...
        auto && test = Detail::run_for_at_least<
            Clock>(std::chrono::duration_cast<IDuration>(run_time), 1, fun);
...
struct StringMaker<signed char[SZ]>
{
    static std::string convert(signed char const * str)
    {
        ...
        return Detail::convertIntoString(StringRef(reinterpreted,
                                                   Detail::catch_strnlen(reinterpreted, SZ)));
    }
};
template <size_t SZ>
...
struct StringMaker<
    R,
    std::enable_if_t<is_range<R>::value && !::Catch::Detail::IsStreamInsertable<R>::value>>
{
...
{
    static std::string convert(std::chrono::time_point<std::chrono::system_clock, Duration> const &
                                   time_point)
    {
...
public:
    ...
    bool matches(TestCaseInfo const & testCase) const;
    Matches matchesByFilter(std::vector<TestCaseHandle> const & testCases, IConfig const & config)
        const;
...
#define INTERNAL_CATCH_INFO(macroName, log)                                                  \
    const Catch::ScopedMessage                                                               \
    INTERNAL_CATCH_UNIQUE_NAME(scopedMessage)(Catch::MessageBuilder(macroName##_catch_sr,    \
                                                                    CATCH_INTERNAL_LINEINFO, \
                                                                    Catch::ResultWas::Info)  \
                                              << log)
...
#define INTERNAL_CATCH_UNSCOPED_INFO(macroName, log)                           \
    Catch::getResultCapture()                                                  \
        .emplaceUnscopedMessage(Catch::MessageBuilder(macroName##_catch_sr,    \
                                                      CATCH_INTERNAL_LINEINFO, \
                                                      Catch::ResultWas::Info)  \
                                << log)
...
public:
    ...
    using ParserBase::parse;
    Detail::InternalParseResult parse(std::string const & exeName, Detail::TokenStream tokens)
        const override;
};
...
#                        define CATCH_DEFINE_COMPARABLE_TRAIT(id, op)                      \
                            ...
                            struct is_##id##_0_comparable<                                 \
                                T,                                                         \
                                void_t<decltype(std::declval<T>() op 0)>> : std::true_type \
                            {};
...
#                        pragma warning( \
                            disable : 4312) // Converting int to T* using reinterpret_cast (issue on
                                            // x64 platform)
...
#                define INTERNAL_CATCH_TEST(macroName, resultDisposition, ...)                                          \
                    ...
                    { /* NOLINT(bugprone-infinite-loop) */                                                              \
                        ...
                        {                                                                                               \
                            ...
                            catchAssertionHandler                                                                       \
                                .handleExpr(Catch::Decomposer() <= __VA_ARGS__); /* NOLINT(bugprone-chained-comparison) \
                                                                                  */                                    \
                            CATCH_INTERNAL_STOP_WARNINGS_SUPPRESSION                                                    \
                        }                                                                                               \
                    ...
                    while ((void)0,                                                                                     \
                           (false)                                                                                      \
                               && static_cast<const bool &>(!!(__VA_ARGS__))) // the expression here
                                                                              // is never evaluated
                                                                              // at runtime but it
                                                                              // forces the compiler
                                                                              // to give it a look
...
#                define INTERNAL_CATCH_SECTION(...)                                               \
                    ...
                    if ([[maybe_unused]] const int                                                \
                            catchInternalPreviousSectionHint = catchInternalSectionHint,          \
...
#                define INTERNAL_CATCH_DYNAMIC_SECTION(...)                                 \
                    ...
                    if ([[maybe_unused]] const int                                          \
                        ...
                        catchInternalSectionHint =                                          \
                            Catch::Detail::GetNewSectionHint((Catch::ReusableStringStream() \
                                                              << __VA_ARGS__)               \
                                                                 .str());                   \
                        catchInternalPreviousSectionHint == __LINE__)                       \
                    CATCH_INTERNAL_STOP_WARNINGS_SUPPRESSION
...
#                define INTERNAL_CATCH_TESTCASE2(fname, ...)                                    \
                    ...
                    static const Catch::Detail::DummyUse                                        \
                        INTERNAL_CATCH_UNIQUE_NAME(dummyUser)(&(fname),                         \
                                                              Catch::NameAndTags{__VA_ARGS__}); \
                    CATCH_INTERNAL_SUPPRESS_SHADOW_WARNINGS                                     \
...
#        elif !defined(CATCH_CONFIG_PREFIX_ALL) && !defined(CATCH_CONFIG_DISABLE) // ^^ prefixed,
                                                                                  // disabled | vv
                                                                                  // unprefixed,
                                                                                  // implemented
...
#        elif !defined(CATCH_CONFIG_PREFIX_ALL) && defined(CATCH_CONFIG_DISABLE) // ^^ unprefixed,
                                                                                 // implemented | vv
                                                                                 // unprefixed,
                                                                                 // disabled
...
#                define INTERNAL_CATCH_DECLARE_SIG_TEST_METHOD(TestName, ClassName, ...) \
                    INTERNAL_CATCH_EXPAND_VARGS(INTERNAL_CATCH_VA_NARGS_IMPL(            \
                        ...
                        INTERNAL_CATCH_DECLARE_SIG_TEST_METHOD_X,                        \
                        INTERNAL_CATCH_DECLARE_SIG_TEST_METHOD1,                         \
                        INTERNAL_CATCH_DECLARE_SIG_TEST_METHOD0)(TestName,               \
                                                                 ClassName,              \
                                                                 __VA_ARGS__))
#                define INTERNAL_CATCH_NTTP_REG_METHOD_GEN(TestName, ...)             \
                    INTERNAL_CATCH_EXPAND_VARGS(INTERNAL_CATCH_VA_NARGS_IMPL(         \
...
#            define CATCH_TEMPLATE_TEST_CASE_METHOD(className, ...)                                \
                INTERNAL_CATCH_EXPAND_VARGS(INTERNAL_CATCH_TEMPLATE_TEST_CASE_METHOD(className,    \
                                                                                     __VA_ARGS__))
#            define CATCH_TEMPLATE_TEST_CASE_METHOD_SIG(className, ...)                   \
                INTERNAL_CATCH_EXPAND_VARGS(                                              \
                    INTERNAL_CATCH_TEMPLATE_TEST_CASE_METHOD_SIG(className, __VA_ARGS__))
...
public:
    ...
    {
        Detail::registerTranslatorImpl(Detail::make_unique<
                                       ExceptionTranslator<T>>(translateFunction));
    }
};
...
GeneratorWrapper<DecayedT> value(T && value)
{
    return GeneratorWrapper<
        DecayedT>(Catch::Detail::make_unique<SingleValueGenerator<DecayedT>>(CATCH_FORWARD(value)));
}
template <typename T>
...
template <typename... Ts>
GeneratorWrapper<std::tuple<std::decay_t<Ts>...>> table(std::initializer_list<
                                                        std::tuple<std::decay_t<Ts>...>> tuples)
{
              ...
              L const & generatorExpression) -> typename decltype(generatorExpression())::type
{
    ...
    if (!tracker)
    {
        tracker = createGeneratorTracker(generatorName,
                                         lineInfo,
                                         Catch::Detail::make_unique<
                                             Generators<UnderlyingType>>(generatorExpression()));
    }

    auto const &
        generator = static_cast<IGenerator<UnderlyingType> const &>(*tracker->getGenerator());
...
{
    return GeneratorWrapper<
        T>(Catch::Detail::make_unique<TakeGenerator<T>>(target, CATCH_MOVE(generator)));
}
...
{
    return GeneratorWrapper<
        T>(Catch::Detail::make_unique<
           FilterGenerator<T, Predicate>>(CATCH_FORWARD(pred), CATCH_MOVE(generator)));
}
...
{
    return GeneratorWrapper<
        T>(Catch::Detail::make_unique<RepeatGenerator<T>>(repeats, CATCH_MOVE(generator)));
}
...
GeneratorWrapper<T> map(Func && function, GeneratorWrapper<U> && generator)
{
    return GeneratorWrapper<
        T>(Catch::Detail::make_unique<
           MapGenerator<T, U, Func>>(CATCH_FORWARD(function), CATCH_MOVE(generator)));
}
...
{
    return GeneratorWrapper<
        std::vector<T>>(Catch::Detail::make_unique<ChunkGenerator<T>>(size, CATCH_MOVE(generator)));
}
...
constexpr std::enable_if_t<std::is_signed<OriginalType>::value, UnsignedType> transposeToNaturalOrder(
    UnsignedType in)
{
...
template <typename OriginalType, typename UnsignedType>
constexpr std::enable_if_t<std::is_unsigned<OriginalType>::value, UnsignedType> transposeToNaturalOrder(
    UnsignedType in)
{
    static_assert(sizeof(OriginalType) == sizeof(UnsignedType),
                  "reordering requires the same sized types on both sides");
...
{
    return GeneratorWrapper<
        T>(Catch::Detail::make_unique<RandomIntegerGenerator<T>>(a, b, Detail::getSeed()));
}
...
{
    return GeneratorWrapper<
        T>(Catch::Detail::make_unique<RandomFloatingGenerator<T>>(a, b, Detail::getSeed()));
}
...
{
    return GeneratorWrapper<
        ResultType>(Catch::Detail::make_unique<IteratorGenerator<ResultType>>(from, to));
}
                      ...
                      std::size_t const shardIndex)
{
    ...
    const std::size_t
        endIndex = (shardIndex + 1) * shardSize + (std::min)(shardIndex + 1, leftoverTests);
...
{
    return static_cast<
        XmlFormatting>(static_cast<std::uint8_t>(lhs) | static_cast<std::uint8_t>(rhs));
}
...
public:
    ...
    std::string describe() const override
    {
        return describe_multi_matcher<
            MatcherTs...>(" and "_sr, m_matchers, std::index_sequence_for<MatcherTs...>{});
    }
...
public:
    ...
    {
        return describe_multi_matcher<
            MatcherTs...>(" or "_sr, m_matchers, std::index_sequence_for<MatcherTs...>{});
    }
...
template <typename Matcher>
std::enable_if_t<Detail::is_matcher<Matcher>::value, SizeMatchesMatcher<Matcher>> SizeIs(Matcher &&
                                                                                             m)
{
 ...
 */
template <typename T>
std::enable_if_t<!Detail::is_matcher<T>::value, ContainsElementMatcher<T, std::equal_to<>>> Contains(
    T && elem)
{
...
template <typename T, typename AllocComp = std::allocator<T>, typename AllocMatch = AllocComp>
UnorderedEqualsMatcher<T, AllocComp, AllocMatch> UnorderedEquals(std::vector<T, AllocComp> const &
                                                                     target)
{
...
private:
    ...
    // NOTE: #1 (manu)
    // I hacked the implementation to prevent displaying the banner
