...
requires std::derived_from<
    T, std::basic_streambuf<typename T::char_type, typename T::traits_type>>
#endif
...
public:
  ...
  inline constexpr native_handle_type native_handle() const noexcept { return fb; }
  ...
  explicit constexpr operator bool() const noexcept { return fb; }
  ...
  explicit operator basic_posix_io_observer<char_type>() const noexcept
  {
    return static_cast<basic_posix_io_observer<char_type>>(
        static_cast<basic_c_io_observer<char_type>>(*this));
  }
...
template<typename T> inline void flush(basic_general_streambuf_io_observer<T> h)
{
  if (h.fb->pubsync() == -1) throw_posix_error(EIO);
}
...
using basic_streambuf_io_observer =
    basic_general_streambuf_io_observer<std::basic_streambuf<CharT, Traits>>;

template<std::integral CharT, typename Traits = std::char_traits<CharT>>
using basic_filebuf_io_observer =
    basic_general_streambuf_io_observer<std::basic_filebuf<CharT, Traits>>;
...
zero_copy_in_handle(basic_filebuf_io_observer<ch_type, Traits> h)
{
...
inline constexpr decltype(auto)
zero_copy_out_handle(basic_filebuf_io_observer<ch_type, Traits> h)
{
  return zero_copy_out_handle(static_cast<basic_c_io_observer<ch_type>>(h));
}

template<std::integral ch_type, typename Traits>
inline auto seek(basic_filebuf_io_observer<ch_type, Traits> h,
                 std::intmax_t offset = 0, seekdir s = seekdir::cur)
{
  h.fb->pubsync();
...
io_control(basic_filebuf_io_observer<ch_type, traits_type> h, Args&&... args)
{
  return io_control(static_cast<basic_c_io_observer<ch_type>>(h),
...
inline constexpr posix_at_entry
at(basic_filebuf_io_observer<char_type, traits_type> other) noexcept
{
  return posix_at_entry{
      details::fp_to_fd(details::streambuf_hack::fp_hack(other.fb))};
}
...
template<typename T> requires (
    std::same_as<
        T, std::basic_streambuf<typename T::char_type, typename T::traits_type>> ||
    std::derived_from<
        T, std::basic_filebuf<typename T::char_type, typename T::traits_type>>)
inline bool is_character_device(basic_general_streambuf_io_observer<T> other)
{
  using char_type = typename T::char_type;
  basic_c_io_observer<char_type> bciob{
      static_cast<basic_c_io_observer<char_type>>(other)};
  if constexpr (
      std::same_as<
          T, std::basic_streambuf<typename T::char_type, typename T::traits_type>>) {
    if (bciob.fp == nullptr)
...
inline void clear_screen(basic_general_streambuf_io_observer<T> other)
{
  ...
  if (!win32::details::win32_is_character_device(handle)) return;
  other.fb->pubsync();
  details::c_flush_unlocked_impl(fp);
  win32::details::win32_clear_screen_main(handle);
    #else
  if (!details::posix_is_character_device(fd)) return;
  other.fb->pubsync();
  details::c_flush_unlocked_impl(fp);
...
template<std::integral char_type, typename traits_type>
inline std::size_t
streambuf_write_report_eh_impl(std::basic_streambuf<char_type, traits_type>* fb,
                               char_type const* src, std::size_t count)
{
  return static_cast<std::size_t>(fb->sputn(src,
                                            static_cast<std::streamsize>(count)));
}

template<std::integral char_type, typename traits_type>
inline std::size_t
streambuf_read_report_eh_impl(std::basic_streambuf<char_type, traits_type>* fb,
                              char_type* dest, std::size_t count)
{
  return static_cast<std::size_t>(fb->sgetn(dest,
                                            static_cast<std::streamsize>(count)));
}
...
template<std::integral char_type, typename traits_type>
inline std::size_t
streambuf_write_impl(basic_streambuf_io_observer<char_type, traits_type> t,
                     char_type const* src, std::size_t count)
{
...
template<std::integral char_type, typename traits_type>
inline std::size_t
streambuf_read_impl(basic_streambuf_io_observer<char_type, traits_type> t,
                    char_type* dest, std::size_t count)
{
...
requires (std::same_as<typename T::char_type, ::std::iter_value_t<Iter>> ||
          std::same_as<typename T::char_type, char>)
[[nodiscard]] inline Iter
read(basic_general_streambuf_io_observer<T> t, Iter begin, Iter end)
{
  using char_type = typename T::char_type;
  using traits_type = typename T::traits_type;
  if constexpr (std::same_as<typename T::char_type, ::std::iter_value_t<Iter>>)
    return begin + details::streambuf_read_impl(
                       basic_streambuf_io_observer<char_type, traits_type>{t.fb},
    ...
    return begin + details::streambuf_read_impl(
                       ...
                       static_cast<std::size_t>(end - begin) * sizeof(*begin)) /
                       (*begin);
}
...
inline Iter write(basic_general_streambuf_io_observer<T> t, Iter begin, Iter end)
{
  ...
  using traits_type = typename T::traits_type;
  if constexpr (std::same_as<typename T::char_type, ::std::iter_value_t<Iter>>)
    return begin + details::streambuf_write_impl(
                       basic_streambuf_io_observer<char_type, traits_type>{t.fb},
                       ::std::to_address(begin),
                       static_cast<std::size_t>(end - begin));
  else
