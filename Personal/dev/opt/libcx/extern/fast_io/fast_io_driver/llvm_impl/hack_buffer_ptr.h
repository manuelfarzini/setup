...
template<std::size_t position> requires (position < 3u)
inline constexpr std::size_t llvm_raw_ostream_cal_obuffer_ptr_position() noexcept
{
  constexpr std::size_t offset{
      __builtin_offsetof(llvm_raw_ostream_model, OutBufStart)};
...
{
  constexpr std::size_t offset{
      llvm_raw_ostream_cal_obuffer_ptr_position<position>()};
  char_type* ptr;
  ::fast_io::details::my_memcpy(__builtin_addressof(ptr),
                                reinterpret_cast<char*>(os) + offset,
                                sizeof(char_type*));
  return ptr;
}

inline void
llvm_raw_ostream_set_obuffer_curr_ptr(::llvm::raw_ostream* os, char* vdptr)
{
  constexpr std::size_t offset{llvm_raw_ostream_cal_obuffer_ptr_position<1>()};
  ::fast_io::details::my_memcpy(reinterpret_cast<char*>(os) + offset,
                                __builtin_addressof(vdptr), sizeof(char*));
}
...
inline int hack_fd_from_llvm_raw_fd_ostream(::llvm::raw_fd_ostream* os) noexcept
{
  ...
  int fd;
  ::fast_io::details::my_memcpy(__builtin_addressof(fd),
                                reinterpret_cast<char*>(os) + fdoffset, sizeof(int));
  return fd;
}

inline void
hack_set_fd_to_llvm_raw_fd_ostream(::llvm::raw_fd_ostream* os, int fd) noexcept
{
  ::fast_io::details::my_memcpy(reinterpret_cast<char*>(os) + fdoffset,
                                __builtin_addressof(fd), sizeof(int));
}
