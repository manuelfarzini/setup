...
{
    for (const auto& w : table) print("{} ", w.first, w.second);
    println("");
}
...
bool word_is_acceptable(vstring const& word, char const** argv)
{
    ...
    for (auto const& c : word) {
        for (size_t t = 0; t < strlen(argv[2]); t++) {
            if (c == argv[2][t]) {
                return false;
            }
        }
...
int main(int argc, char const** argv)
{
    ...
    if (argc != 3) {
        println(
            "Usage: ./typing <characters_must_have> <characters_must_filter_out>");
        println("Note: the arguments must be two contiguous strings");
        return 1;
    }

    std::ifstream file("/Users/manuelfarzini/Personal/dev/doc/typing/typing.input");
    if (!file) {
    ...
    while (file >> word) {
        ...
        if (!word_is_acceptable(word, argv)) {
            continue;
        }
